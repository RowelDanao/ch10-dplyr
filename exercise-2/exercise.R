...
two_wheel_plus20mpg <- filter(vehicles, drive == '2-Wheel Drive', cty > 20)

# Of the above vehicles, what is the vehicle ID of the vehicle with the worst 
# hwy mpg?
# Hint: filter for the worst vehicle, then select its ID.
filtered <- filter(two_wheel_plus20mpg, hwy == min(hwy))
...
desired_vehicle <- function(make_choice, year_choice){
